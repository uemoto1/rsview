...
// 2023/08 重要メモ
//  origin_center メンバ変数を初期化時にセットすること
...
class Crystal3D {
    ...
    constructor(canvas) {
        ...
        // 照明
        const light_a = new THREE.AmbientLight(0xFFFFFF, 1.00);
        const light_d = new THREE.DirectionalLight(0xFFFFFF, 0.80);
        ...
        // フォッグ
        this.scene.fog = new THREE.Fog(0xFFFFFF, 4, 6);
    ...
    plot() {
        ...
        // 基準位置計算
        const o1 = this.origin_center ? 0 : -0.5*n1;
        const o2 = this.origin_center ? 0 : -0.5*n2;
        const o3 = this.origin_center ? 0 : -0.5*n3;
        // 原子球の配置
    ...
    to_cif() {
        var offset = (this.origin_center) ? -0.5: 0.0;
        ...
        for (var i = 0; i < this.atom_data.length; i++) {
            const t1 = this.atom_data[i].t1 + offset;
            const t2 = this.atom_data[i].t2 + offset;
            const t3 = this.atom_data[i].t3 + offset;
            const iz = this.atom_data[i].iz;
            const symbol = atom_symbol_table[iz];
            tmp += symbol + " " + t1.toFixed(9) + " " + t2.toFixed(9) + " " + t3.toFixed(9) + "\n";
        }
        return tmp;
    }
};
